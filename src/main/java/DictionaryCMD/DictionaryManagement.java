...
public class DictionaryManagement {
    ...
    public static ObservableList<Word> searchFromDatabase(String wordToFind) throws SQLException {
        ...
        statement = connection.createStatement();
        try {
            String sql = "SELECT * FROM av WHERE word like " + "'" + wordToFind + "%'" + " ORDER BY word limit 100";
    ...
    public static void addToBookmark(Word word) throws IOException, SQLException {
        ...
        connection = DriverManager.getConnection("jdbc:sqlite:./src/main/resources/db/dict_hh.db");
        try {
            String sql = "UPDATE av set bookmark = ? where word = ?";
            preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setInt(1, 1);
            preparedStatement.setString(2, word.getWord_target());
    ...
    public static void deleteFromBookmark(Word word) throws IOException, SQLException {
        ...
        statement = connection.createStatement();
        try {
            String sql = "UPDATE av set bookmark = NULL where word = '" + word.getWord_target() + "'";
            statement.executeUpdate(sql);
            connection.close();
            statement.close();
        } catch (Exception e) {
